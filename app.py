...
# Mendapatkan nilai variabel dari environment
password = os.getenv('DB_PASSWORD')
cxn_str = os.getenv('DB_URI')
...
def detail(keyword):
    api_key = "33d9dcc8-8e85-4062-b60d-d883b8bfe475"
    ...
    if not definitions:
        return render_template(
            'error.html', 
            msg=f"Could not find the word '{keyword}'. Please try again.",
            suggestions=[]
        )
    
    if isinstance(definitions[0], str):
        suggestions = [word for word in definitions]
        return render_template(
            'error.html',
